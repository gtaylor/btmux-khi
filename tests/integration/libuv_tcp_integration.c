...
static int write_lua_fixture(const char *directory) {
  ...
  fputs("local btech_api = {\n"
        "  unit = \"armor battle_value critical_slots engine weapons "
        "installed_parts payload technologies preferred_id set_preferred_id "
        "markings set_markings assigned_pilot set_assigned_pilot display_name "
        "set_display_name set_armor apply_damage radio_channels load_template "
        "piloting_check effective_max_speed effective_max_speed_kph "
        "section_condition set_max_speed set_tonnage tic_weapons restore "
        "save_template reset_critical_slots install_weapon "
        "configure_ammunition restock_ammunition set_weapon_modes "
        "install_special add_technology remove_technology "
        "clear_technologies set_unit_type set_movement_type set_jump_speed "
        "set_heat_sinks set_long_range_sensor_range set_tactical_range "
        "set_scan_range set_radio_quality set_radio_range "
        "set_cargo_capacity\",\n"
        "  template = \"exists armor battle_value critical_slots engine "
        "weapons installed_parts payload technologies base_cost show_status "
        "show_weapon_specs show_critical_status\",\n"
        "  map = \"blast_zones cargo_transfer_point set_cargo_transfer_point "
        "elevation terrain emit in_blast_zone line_of_sight unit_by_id link "
        "set_link load range place_unit units update_links\",\n"
        "  player = \"loadout mechwarrior_template set_loadout "
        "set_mechwarrior_template set_ui_preferences ui_preferences\",\n"
        "  parts = \"categories list search resolve stores store_quantity "
        "adjust_stores set_cost\",\n"
        "  character = \"catalog value experience_threshold set_value "
        "set_skill_target set_skill_experience add_skill_experience\",\n"
        "  repair = \"apply needs is_under_repair is_fixable "
        "technician_available_in\",\n"
        "  autopilot = \"status add_order remove_order clear_orders engage "
        "disengage event_stats\",\n"
        "  system = \"event_lag units_in_zone\",\n"
        "}\n"
        ...
        "  end\n"
        "  local namespaces = {\n"
        "    autopilot = \"orders directions roam_modes autogun_modes\",\n"
        "    unit = \"types movement_types sections technology "
        "technology_groups fire_modes ammunition_modes\",\n"
        "    repair = \"operations\",\n"
        "  }\n"
        "  for name in (namespaces[pkg] or \"\"):gmatch(\"%S+\") do\n"
        "    expected[name] = true\n"
        "    assert(type(btech[pkg][name]) == \"userdata\")\n"
        "  end\n"
        "  for name in pairs(btech[pkg]) do assert(expected[name]) end\n"
        "end\n"
        "local btech_roots = { autopilot = true, character = true, error = "
        "true, map = true, "
        "parts = true, player = true, repair = true, system = true, "
        "template = true, unit = true }\n"
  ...
  fputs("        assert(type(mux.comsys) == \"table\")\n"
        ...
        "        assert(type(api_object.destination) == \"function\")\n"
        "        assert(type(api_object.set_destination) == \"function\")\n"
        "        assert(type(api_object.home) == \"function\")\n"
        "        assert(type(api_object.set_home) == \"function\")\n"
        "        assert(type(api_object.location) == \"function\")\n"
        "        assert(api_object.link_exit == nil)\n"
        "        assert(mux.world.destination == nil)\n"
        "        assert(mux.world.set_destination == nil)\n"
        "        assert(mux.world.home == nil)\n"
        "        assert(mux.world.set_home == nil)\n"
        "        assert(mux.world.location == nil)\n"
        "        assert(type(mux.world.teleport_object) == \"function\")\n"
        "        assert(type(mux.world.destroy_object) == \"function\")\n",
        file);
  fputs(
      ...
      "        assert(type(getmetatable(mux.comsys.flags)) == \"string\")\n"
      "        assert(type(getmetatable(public)) == \"string\")\n"
      "        assert(not pcall(setmetatable, {}, "
      "getmetatable(mux.comsys.flags)))\n"
      "        assert(not pcall(setmetatable, {}, getmetatable(public)))\n"
      ...
      "        local mutate_flag_namespace_ok, mutate_flag_namespace_error =\n"
      "          mux.error.pcall(function()\n"
      "            mux.comsys.flags.PUBLIC = loud\n"
      "          end)\n"
      "        assert(not mutate_flag_namespace_ok and\n"
      "          mutate_flag_namespace_error.code == "
      "\"mux.channel_flag.invalid\")\n"
      "        assert(public == mux.comsys.flags.PUBLIC)\n"
      ...
      file);
  fputs(
      "        local quiet_count = channel:message_count()\n"
      "        channel:add_player(api_object, \"lq\", true)\n"
      "        assert(channel:user_count() == 1)\n"
      "        assert(#channel:who() == 1)\n"
      "        assert(channel:message_count() == quiet_count)\n"
      "        local alias_ok, alias_error = mux.error.pcall(\n"
      "          channel.add_player, channel, api_object, \"lq\", true)\n"
      "        assert(not alias_ok and alias_error.code == "
      "\"mux.arg.invalid\")\n"
      "        local invalid_alias_ok, invalid_alias_error = mux.error.pcall(\n"
      "          channel.add_player, channel, api_object, \"bad alias\", "
      "true)\n"
      "        assert(not invalid_alias_ok and\n"
      "          invalid_alias_error.code == \"mux.arg.invalid\")\n"
      "        local quiet_ok, quiet_error = mux.error.pcall(\n"
      "          channel.add_player, channel, api_object, \"lm\", \"yes\")\n"
      "        assert(not quiet_ok and quiet_error.code == "
      "\"mux.arg.invalid\")\n"
      "        local missing_quiet_ok, missing_quiet_error = mux.error.pcall(\n"
      "          channel.add_player, channel, api_object, \"lm\")\n"
      "        assert(not missing_quiet_ok and\n"
      "          missing_quiet_error.code == \"mux.arg.invalid\")\n"
      "        local non_player = mux.world.create_object({\n"
      "          type = mux.world.types.ROOM, name = \"Lua Comsys Room\"\n"
      "        })\n"
      "        local player_ok, player_error = mux.error.pcall(\n"
      "          channel.add_player, channel, non_player, \"lr\", true)\n"
      "        assert(not player_ok and\n"
      "          player_error.code == \"mux.object.invalid\")\n"
      "        mux.world.destroy_object(non_player)\n"
      "        channel:boot_player(api_object)\n"
      "        local announced_count = channel:message_count()\n"
      "        channel:add_player(api_object:dbref(), \"la\", false)\n"
      "        assert(channel:user_count() == 1)\n"
      "        assert(channel:message_count() == announced_count + 1)\n"
      "        local existing_count = channel:message_count()\n"
      "        channel:add_player(api_object, \"la2\", false)\n"
      "        assert(channel:user_count() == 1)\n"
      "        assert(channel:message_count() == existing_count)\n",
      file);
  fputs(
  ...
  fputs("    {\n"
        ...
        "        })\n"
        "        linked_exit:set_destination(link_destination_one)\n"
        "        linked_exit:set_destination(link_destination_two)\n"
        "        unlinked_exit:set_destination(nil)\n"
        "        assert(exit:destination() == room)\n"
        "        assert(linked_exit:destination() == link_destination_two)\n"
        "        assert(unlinked_exit:destination() == nil)\n"
        "        safe_thing = mux.world.create_object({ type = "
        "mux.world.types.THING,\n"
        "          name = \"Lua Safe\", location = ctx.enactor\n"
        "        })\n"
        "        local object_types = mux.world.types\n"
        "        local room_type = object_types.ROOM\n"
        "        assert(type(getmetatable(object_types)) == \"string\")\n"
        "        assert(type(getmetatable(room_type)) == \"string\")\n"
        "        assert(not pcall(setmetatable, {}, "
        "getmetatable(object_types)))\n"
        "        assert(not pcall(setmetatable, {}, getmetatable(room_type)))\n"
        "        local type_write_ok, type_write_error = "
        "mux.error.pcall(function()\n"
        "          room_type.value = 1\n"
        "        end)\n"
        "        assert(not type_write_ok and type_write_error.code == "
        "\"mux.arg.invalid\")\n"
        "        local types_write_ok, types_write_error = "
        "mux.error.pcall(function()\n"
        "          object_types.ROOM = object_types.THING\n"
        "        end)\n"
        "        assert(not types_write_ok and types_write_error.code == "
        "\"mux.arg.invalid\")\n"
        "        assert(room_type == object_types.ROOM and "
        "tostring(room_type) == \"ROOM\")\n"
  ...
  fputs(
      ...
      "        local player_home = player:home()\n"
      "        assert(player_home ~= nil and player:location() ~= nil)\n"
      "        player:set_home(player_home)\n"
      "        assert(player:home() == player_home)\n"
      "        assert(thing:home() == room and thing:location() == room)\n"
      "        assert(mux.world.object(thing:dbref()):home() == room)\n"
      "        assert(mux.world.object(thing:dbref()):location() == room)\n"
      "        thing:set_home(link_destination_one)\n"
      "        assert(thing:home() == link_destination_one)\n"
      "        thing:set_home(link_destination_two:dbref())\n"
      "        assert(thing:home() == link_destination_two)\n"
  ...
  fputs("        local zoned_kinds = mux.world.list_objects({\n"
        ...
        "        mux.world.destroy_object(empty_list_zone)\n"
  ...
  fputs("        local missing_ok, missing_error = mux.error.pcall(\n"
        "          linked_exit.set_destination, linked_exit)\n"
        "        assert(not missing_ok and missing_error.code == "
        "\"mux.arg.invalid\")\n"
        "        local source_ok, source_error = mux.error.pcall(\n"
        "          room.set_destination, room, nil)\n"
        "        assert(not source_ok and source_error.code == "
        "\"mux.object.invalid\")\n"
        "        local thing_source_ok, thing_source_error = mux.error.pcall(\n"
        "          thing.set_destination, thing, nil)\n"
        "        assert(not thing_source_ok and thing_source_error.code == "
        "\"mux.object.invalid\")\n"
        "        local player_source_ok, player_source_error = "
        "mux.error.pcall(\n"
        "          player.set_destination, player, nil)\n"
        "        assert(not player_source_ok and player_source_error.code == "
        "\"mux.object.invalid\")\n"
        "        local destination_room_ok, destination_room_error =\n"
        "          mux.error.pcall(room.destination, room)\n"
        "        assert(not destination_room_ok and\n"
        "          destination_room_error.code == \"mux.object.invalid\")\n"
        "        local home_room_ok, home_room_error = mux.error.pcall(\n"
        "          room.home, room)\n"
        "        assert(not home_room_ok and\n"
        "          home_room_error.code == \"mux.object.invalid\")\n"
        "        local home_exit_ok, home_exit_error = mux.error.pcall(\n"
        "          exit.home, exit)\n"
        "        assert(not home_exit_ok and\n"
        "          home_exit_error.code == \"mux.object.invalid\")\n"
        "        local location_room_ok, location_room_error =\n"
        "          mux.error.pcall(room.location, room)\n"
        "        assert(not location_room_ok and\n"
        "          location_room_error.code == \"mux.object.invalid\")\n"
        "        local location_exit_ok, location_exit_error =\n"
        "          mux.error.pcall(exit.location, exit)\n"
        "        assert(not location_exit_ok and\n"
        "          location_exit_error.code == \"mux.object.invalid\")\n"
        "        local missing_home_ok, missing_home_error = mux.error.pcall(\n"
        "          thing.set_home, thing)\n"
        "        assert(not missing_home_ok and\n"
        "          missing_home_error.code == \"mux.arg.invalid\")\n"
        "        local nil_home_ok, nil_home_error = mux.error.pcall(\n"
        "          thing.set_home, thing, nil)\n"
        "        assert(not nil_home_ok and\n"
        "          nil_home_error.code == \"mux.object.invalid\")\n"
        "        local exit_home_ok, exit_home_error = mux.error.pcall(\n"
        "          thing.set_home, thing, exit)\n"
        "        assert(not exit_home_ok and\n"
        "          exit_home_error.code == \"mux.object.invalid\")\n"
        "        local self_home_ok, self_home_error = mux.error.pcall(\n"
        "          thing.set_home, thing, thing)\n"
        "        assert(not self_home_ok and\n"
        "          self_home_error.code == \"mux.object.invalid\")\n"
        "        local room_set_home_ok, room_set_home_error =\n"
        "          mux.error.pcall(room.set_home, room, link_destination_one)\n"
        "        assert(not room_set_home_ok and\n"
        "          room_set_home_error.code == \"mux.object.invalid\")\n"
        "        local target_ok, target_error = mux.error.pcall(\n"
        "          linked_exit.set_destination, linked_exit, exit)\n"
  ...
  fputs(
      ...
      "        })\n"
      "        linked_exit:set_destination(going_destination)\n"
      "        mux.world.destroy_object(going_destination)\n"
      "        assert(linked_exit:destination() == nil)\n"
      "        local destination_ok, destination_error = mux.error.pcall(\n"
      "          linked_exit.set_destination, linked_exit, going_destination)\n"
      "        assert(not destination_ok and destination_error.code == "
      "\"mux.object.unavailable\")\n"
      "        linked_exit:set_destination(link_destination_two)\n"
      "        local going_home = mux.world.create_object({ type =\n"
      "          mux.world.types.THING, name = \"Lua Going Home\",\n"
      "          location = room })\n"
      "        thing:set_home(going_home)\n"
      "        mux.world.destroy_object(going_home)\n"
      "        assert(thing:home() == nil)\n"
      "        local going_home_ok, going_home_error = mux.error.pcall(\n"
      "          thing.set_home, thing, going_home)\n"
      "        assert(not going_home_ok and\n"
      "          going_home_error.code == \"mux.object.unavailable\")\n"
      "        local location_room = mux.world.create_object({ type =\n"
      "          mux.world.types.ROOM, name = \"Lua Going Location\" })\n"
      "        local location_thing = mux.world.create_object({ type =\n"
      "          mux.world.types.THING, name = \"Lua Located Thing\",\n"
      "          location = location_room })\n"
      "        mux.world.destroy_object(location_room)\n"
      "        assert(location_thing:location() == nil)\n"
      "        local going_home_receiver = mux.world.create_object({\n"
      "          type = mux.world.types.THING,\n"
      "          name = \"Lua Going Home Receiver\", location = room })\n"
      "        mux.world.destroy_object(going_home_receiver)\n"
      "        local going_home_receiver_ok, going_home_receiver_error =\n"
      "          mux.error.pcall(going_home_receiver.set_home,\n"
      "            going_home_receiver, room)\n"
      "        assert(not going_home_receiver_ok and\n"
      "          going_home_receiver_error.code ==\n"
      "            \"mux.object.unavailable\")\n"
      "        local going_exit = mux.world.create_object({ type = "
      "mux.world.types.EXIT,\n"
      "          name = \"Lua Going Exit\", location = room\n"
      "        })\n"
      "        mux.world.destroy_object(going_exit)\n"
      "        local going_ok, going_error = mux.error.pcall(\n"
      "          going_exit.set_destination, going_exit, nil)\n"
      "        assert(not going_ok and going_error.code == "
      "\"mux.object.unavailable\")\n"
      "        local persistent_home_thing = mux.world.create_object({\n"
      "          type = mux.world.types.THING,\n"
      "          name = \"Lua Persistently Homed Thing\", location = room\n"
      "        })\n"
  ...
  fputs(
      ...
      "        mux.world.destroy_object(list_zone)\n"
      "        mux.world.destroy_object(other_list_zone)\n"
      "        mux.world.destroy_object(thing)\n"
      "        stale_thing = thing\n"
      "        stale_dbref = thing:dbref()\n"
      "        mux.world.destroy_object(styled_room)\n"
      ...
      "          mux.world.destroy_object, ctx.enactor)\n"
  ...
  fputs(
      ...
      "          mux.world.destroy_object, safe_thing)\n"
      "        assert(not safe_ok and safe_error.code == "
      "\"mux.object.unavailable\")\n"
      "        mux.world.destroy_object(safe_thing, { override = true })\n"
  ...
  fputs(
      ...
      "        local result = mux.world.teleport_object({\n"
      ...
      "          mux.world.teleport_object, { object = thing,\n"
      ...
      "          mux.world.teleport_object, { object = source,\n"
      ...
      "          mux.world.teleport_object, { object = thing, destination = "
      "exit })\n"
      "        assert(not exit_ok\n"
      ...
      "          mux.world.teleport_object, { object = container,\n"
      "            destination = child })\n"
      "        assert(not cycle_ok\n"
      ...
      "        })\n"
      "        mux.world.destroy_object(going_object)\n"
      "        local object_ok, object_error = mux.error.pcall(\n"
      "          mux.world.teleport_object, { object = going_object,\n"
      "            destination = destination })\n"
  ...
  fputs("        target:set_zone(room_zone)\n"
        ...
        "        })\n"
        "        mux.world.destroy_object(going_zone)\n"
        ...
        "        })\n"
        "        mux.world.destroy_object(going_target)\n"
        "        local target_ok, target_error = mux.error.pcall(\n"
        ...
        "        mux.world.destroy_object(cleanup_zone)\n"
  ...
  fputs("    {\n"
        ...
        "          mux.world.destroy_object(replacement)\n"
        "        end\n"
  ...
  fputs(
      ...
      "        })\n"
      "        mux.world.destroy_object(going_affiliate)\n"
      ...
      "        mux.world.destroy_object(cleanup_affiliate)\n"
  ...
  fputs(
      ...
      "        })\n"
      "        mux.world.destroy_object(going)\n"
      "        local going_ok, going_error = mux.error.pcall(\n"
  ...
  fputs(
      ...
      "        state:set(\"explicit_nil\", \"temporary\")\n"
      "        state:set(\"explicit_nil\", nil)\n"
      "        assert(not state:has(\"explicit_nil\"))\n"
      "        state:set(\"omitted_value\", \"temporary\")\n"
      "        local missing_state_ok, missing_state_error =\n"
      "          mux.error.pcall(state.set, state, \"omitted_value\")\n"
      "        assert(not missing_state_ok and\n"
      "          missing_state_error.code == \"mux.state.invalid\")\n"
      "        assert(state:get(\"omitted_value\") == \"temporary\")\n"
      "        state:set(\"omitted_value\", nil)\n"
  ...
  fputs(
      ...
      "        assert(type(getmetatable(mux.world.flags)) == \"string\")\n"
      "        assert(type(getmetatable(safe)) == \"string\")\n"
      "        assert(type(getmetatable(mux.world.powers)) == \"string\")\n"
      "        assert(type(getmetatable(idle)) == \"string\")\n"
      "        assert(not pcall(setmetatable, {}, "
      "getmetatable(mux.world.flags)))\n"
      "        assert(not pcall(setmetatable, {}, getmetatable(safe)))\n"
      "        assert(not pcall(setmetatable, {}, "
      "getmetatable(mux.world.powers)))\n"
      "        assert(not pcall(setmetatable, {}, getmetatable(idle)))\n"
      ...
      "\"mux.power.invalid\")\n"
      "        local flag_write_ok, flag_write_error = "
      "mux.error.pcall(function()\n"
      "          safe.value = 1\n"
      "        end)\n"
      "        assert(not flag_write_ok and flag_write_error.code == "
      "\"mux.arg.invalid\")\n"
      "        local power_write_ok, power_write_error = "
      "mux.error.pcall(function()\n"
      "          idle.value = 1\n"
      "        end)\n"
      "        assert(not power_write_ok and power_write_error.code == "
      "\"mux.arg.invalid\")\n"
      "        local flag_namespace_ok, flag_namespace_error = "
      "mux.error.pcall(function()\n"
      "          mux.world.flags.SAFE = transparent\n"
      "        end)\n"
      "        assert(not flag_namespace_ok and flag_namespace_error.code == "
      "\"mux.flag.invalid\")\n"
      "        assert(safe == mux.world.flags.SAFE and idle == "
      "mux.world.powers.IDLE)\n"
      "        mux.world.destroy_object(target)\n"
  ...
  fputs(
      ...
      "      handler = function(ctx)\n"
      "        local player = mux.world.object(ctx.enactor)\n"
      "        local defaults = btech.player.ui_preferences(player)\n"
      "        assert(not defaults.configured and defaults.tactical_height == "
      "14)\n"
      ...
      "        assert(select(\"#\", btech.player.set_ui_preferences("
      "ctx.enactor, preferences)) == 0)\n"
      "        local stored = btech.player.ui_preferences(player)\n"
      "        assert(stored.configured and stored.tactical_width == 22\n"
      "          and stored.buildings == \"include\")\n"
      "        stored.tactical_width = 23\n"
      "        assert(select(\"#\", btech.player.set_ui_preferences("
      "player, stored)) == 0)\n"
      "        stored = btech.player.ui_preferences(player)\n"
      "        assert(stored.configured and stored.tactical_width == 23)\n"
      "        local invalid = {}\n"
      ...
      "        stored = btech.player.ui_preferences(player)\n"
      "        assert(stored.configured and stored.tactical_width == 23)\n"
      "        ok, err = mux.error.pcall("
      "btech.player.set_mechwarrior_template, player, \"Missing\")\n"
      "        assert(not ok and err.code == \"btech.template.not_found\" "
      "and btech.player.mechwarrior_template(ctx.enactor) == nil)\n"
      "        assert(select(\"#\", btech.player.set_loadout(player, nil)) == "
      "0)\n"
      "        assert(btech.player.loadout(player) == nil)\n"
      "        assert(select(\"#\", "
      "btech.player.set_mechwarrior_template(player, nil)) == 0)\n"
      "        assert(btech.player.mechwarrior_template(player) == nil)\n"
      "        assert(select(\"#\", btech.player.set_ui_preferences(player, "
      "nil)) == 0)\n"
      "        assert(not btech.player.ui_preferences(player).configured)\n"
  ...
  fputs(
      ...
      "        local options_ok, options_error = mux.error.pcall(\n"
      "          btech.template.battle_value, \"AS7-D\", {})\n"
      "        assert(not options_ok and options_error.code == "
      "\"mux.arg.invalid\")\n"
      "        local template = btech.template.battle_value(\"BV-Test\")\n"
      "        assert(template.total == template.offensive + "
      "template.defensive\n"
      "          and template.offensive == 20 and template.defensive > 0\n"
      "          and template.defensive ~= math.floor(template.defensive))\n"
      "        assert(select(\"#\", btech.unit.load_template(unit, "
      "\"BV-Test\")) == 0)\n"
      "        local initial = btech.unit.battle_value(unit)\n"
      "        assert(type(initial.total) == \"number\"\n"
      "          and type(initial.offensive) == \"number\"\n"
      "          and type(initial.defensive) == \"number\"\n"
      "          and initial.total == initial.offensive + initial.defensive\n"
      "          and initial.total == template.total\n"
      "          and initial.offensive == template.offensive\n"
      "          and initial.defensive == template.defensive)\n"
      "        for key in pairs(initial) do\n"
      "          assert(key == \"total\" or key == \"offensive\"\n"
      "            or key == \"defensive\")\n"
      "        end\n"
      "        assert(select(\"#\", btech.unit.set_tonnage(unit, 100)) == 0)\n"
      "        local changed = btech.unit.battle_value(unit)\n"
      "        assert(changed.total > initial.total\n"
      "          and changed.offensive > initial.offensive)\n"
      "        assert(btech.unit.preferred_id(unit) == nil)\n"
      "        assert(select(\"#\", btech.unit.set_preferred_id(unit_dbref, "
      "\"bk\")) == 0)\n"
      "        assert(btech.unit.preferred_id(unit) == \"BK\")\n"
      "        assert(select(\"#\", btech.unit.set_markings(unit, "
      "\"stripes\")) == 0)\n"
      "        assert(btech.unit.markings(unit_dbref) == \"stripes\")\n"
      "        local pilot = mux.world.object(ctx.enactor)\n"
      "        assert(select(\"#\", btech.unit.set_assigned_pilot(unit, "
      "pilot)) == 0)\n"
      ...
      "        assert(select(\"#\", btech.unit.set_assigned_pilot(unit, nil)) "
      "== 0)\n"
      "        assert(btech.unit.assigned_pilot(unit) == nil)\n"
      "        assert(select(\"#\", btech.unit.set_markings(unit, nil)) == 0)\n"
      "        assert(select(\"#\", btech.unit.set_preferred_id(unit, nil)) == "
      "0)\n"
      "        assert(btech.unit.markings(unit) == nil\n"
      "          and btech.unit.preferred_id(unit) == nil)\n",
      file);
  fputs(
      "        local left_arm = btech.unit.sections.LEFT_ARM\n"
      "        local center_torso = btech.unit.sections.CENTER_TORSO\n"
      "        assert(tostring(left_arm) == \"LEFT_ARM\"\n"
      "          and left_arm == btech.unit.sections.LEFT_ARM\n"
      "          and left_arm ~= btech.unit.types.MECH)\n"
      "        ok, err = mux.error.pcall(function()\n"
      "          btech.unit.sections.LEFT_ARM = left_arm\n"
      "        end)\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\")\n"
      "        ok, err = mux.error.pcall(function() left_arm.extra = 1 end)\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\")\n"
      "        ok, err = mux.error.pcall(function()\n"
      "          return btech.unit.sections.UNKNOWN\n"
      "        end)\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\")\n"
      "        local repair_operation_names = {\n"
      "          \"REATTACH\", \"REPAIR_PART\", \"REPAIR_WEAPON_TEMPORARY\",\n"
      "          \"REPAIR_ENHANCEMENT\", \"REPAIR_FOCUS\", "
      "\"REPAIR_CRYSTAL\",\n"
      "          \"REPAIR_BARREL\", \"REPAIR_AMMO_FEED\", \"REPAIR_RANGING\",\n"
      "          \"REPAIR_AMMO_MOUNT\", \"REPLACE_WEAPON\", \"RELOAD\",\n"
      "          \"REPAIR_ARMOR\", \"REPAIR_REAR_ARMOR\", "
      "\"REPAIR_INTERNAL\",\n"
      "          \"DETACH\", \"SCRAP_PART\", \"SCRAP_WEAPON\", \"UNLOAD\",\n"
      "          \"RESEAL\", \"REPLACE_SUIT\"\n"
      "        }\n"
      "        for _, name in ipairs(repair_operation_names) do\n"
      "          assert(tostring(btech.repair.operations[name]) == name)\n"
      "        end\n"
      "        ok, err = mux.error.pcall(btech.unit.armor, unit, \"LA\")\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2)\n"
      "        local template_armor = btech.template.armor(\"BV-Test\",\n"
      "          left_arm)\n"
      "        assert(template_armor.section == left_arm)\n"
      "        ok, err = mux.error.pcall(\n"
      "          btech.template.armor, \"BV-Test\", \"LA\")\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2)\n"
      "        assert(select(\"#\", btech.unit.set_armor(unit, left_arm, "
      "{ armor = 12, internal = 6 })) == 0)\n"
      "        local armor = btech.unit.armor(unit, left_arm)\n"
      "        assert(armor.section == left_arm)\n"
      "        assert(armor.armor.current == 12 and armor.armor.original == "
      "12\n"
      "          and armor.internal.current == 6\n"
      "          and armor.internal.original == 6)\n"
      "        assert(select(\"#\", btech.repair.apply(unit, {\n"
      "          operation = btech.repair.operations.REPAIR_ARMOR,\n"
      "          section = left_arm, value = 9\n"
      "        })) == 0)\n"
      "        armor = btech.unit.armor(unit, left_arm)\n"
      "        assert(armor.armor.current == 9 and armor.armor.original == "
      "12)\n"
      "        local found_armor_need = false\n"
      "        for _, need in ipairs(btech.repair.needs(unit)) do\n"
      "          if need.operation == btech.repair.operations.REPAIR_ARMOR\n"
      "              and need.section == left_arm then\n"
      "            found_armor_need = true\n"
      "          end\n"
      "        end\n"
      "        assert(found_armor_need)\n"
      "        ok, err = mux.error.pcall(btech.repair.apply, unit, {\n"
      "          operation = \"repair_armor\", section = left_arm, value = 9\n"
      "        })\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2)\n"
      "        ok, err = mux.error.pcall(btech.repair.apply, unit, {\n"
      ...
      "          and err.detail.argument == 2)\n",
      file);
  fputs("        ok, err = mux.error.pcall(btech.repair.apply, unit)\n"
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 2\n"
        "          and btech.unit.armor(unit, left_arm).armor.current == 9)\n"
        "        ok, err = mux.error.pcall(btech.repair.apply, unit, 17)\n"
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 2\n"
        "          and btech.unit.armor(unit, left_arm).armor.current == 9)\n"
        "        ok, err = mux.error.pcall(btech.repair.apply, unit, \"bad\")\n"
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 2\n"
        "          and btech.unit.armor(unit, left_arm).armor.current == 9)\n"
        "        assert(select(\"#\", btech.repair.apply(unit, {\n"
        ...
        "          section = left_arm, slot = 1, "
        "value = 1\n"
        "        })\n"
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 2)\n"
        "        ok, err = mux.error.pcall(btech.repair.apply, unit, {\n"
        "          operation = btech.repair.operations.REATTACH,\n"
        "          section = left_arm, slot = 1\n"
        "        })\n"
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 2)\n"
        "        ok, err = mux.error.pcall(btech.repair.apply, unit, {\n"
        "          operation = btech.repair.operations.REPAIR_ARMOR,\n"
        "          section = btech.unit.sections.LEFT_SIDE, "
        "value = 8\n"
        "        })\n"
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 2\n"
  ...
  fputs(
      "        local torso_rear =\n"
      "          btech.unit.armor(unit, center_torso).rear_armor.current\n"
      "        local changed_torso_rear = torso_rear == 255 and 254\n"
      "          or torso_rear + 1\n"
      "        assert(select(\"#\", btech.repair.apply(unit, {\n"
      "          operation = btech.repair.operations.REPAIR_REAR_ARMOR,\n"
      "          section = center_torso,\n"
      "          value = changed_torso_rear\n"
      "        })) == 0)\n"
      "        assert(btech.unit.armor(unit, center_torso).rear_armor.current\n"
      "          == changed_torso_rear)\n"
      "        assert(select(\"#\", btech.repair.apply(unit, {\n"
      "          operation = btech.repair.operations.REPAIR_REAR_ARMOR,\n"
      "          section = center_torso,\n"
      "          value = torso_rear\n"
      "        })) == 0)\n"
      "        local arm_rear = btech.unit.armor(unit, "
      "left_arm).rear_armor.current\n"
      "        ok, err = mux.error.pcall(btech.repair.apply, unit, {\n"
      "          operation = btech.repair.operations.REPAIR_REAR_ARMOR,\n"
      "          section = left_arm, value = "
      "7\n"
      ...
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2\n"
      "          and btech.unit.armor(unit, left_arm).rear_armor.current\n"
      "            == arm_rear)\n"
      "        assert(select(\"#\", btech.unit.set_max_speed(unit, 4)) == 0)\n"
      "        local speed_mp = btech.unit.effective_max_speed(unit)\n"
      "        local speed_kph = btech.unit.effective_max_speed_kph(unit)\n"
      "        assert(math.abs(speed_kph / speed_mp - 10.75) < 0.001)\n"
      "        assert(select(\"#\", btech.unit.set_jump_speed(unit, 3)) == 0)\n"
      "        assert(select(\"#\", btech.unit.set_heat_sinks(unit, 12)) == "
      "0)\n"
      "        assert(select(\"#\", "
      "btech.unit.set_long_range_sensor_range(unit, 25)) == 0)\n"
      "        assert(select(\"#\", btech.unit.set_tactical_range(unit, 12)) "
      "== 0)\n"
      "        assert(select(\"#\", btech.unit.set_scan_range(unit, 8)) == 0)\n"
      "        assert(select(\"#\", btech.unit.set_radio_quality(unit, 3)) "
      "== 0)\n"
      "        assert(select(\"#\", btech.unit.set_radio_range(unit, 40)) "
      "== 0)\n"
      "        assert(select(\"#\", btech.unit.set_cargo_capacity(unit, 0, "
      "20)) == 0)\n"
      "        assert(select(\"#\", btech.unit.set_unit_type(unit,\n"
      "          btech.unit.types.MECH)) "
      "== 0)\n"
      "        assert(select(\"#\", "
      "btech.unit.set_movement_type(unit, btech.unit.movement_types.BIPED)) "
      "== 0)\n"
      "        ok, err = mux.error.pcall(btech.unit.set_unit_type, unit, "
      "\"Mech\")\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2)\n"
      "        ok, err = mux.error.pcall(btech.unit.set_movement_type, unit, "
      "\"Biped\")\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2)\n"
      "        ok, err = mux.error.pcall(btech.unit.set_movement_type, unit,\n"
  ...
  fputs(
      "        assert(select(\"#\", "
      "btech.unit.add_technology(unit, btech.unit.technology.SEARCH_LIGHT)) "
      "== 0)\n"
      "        local found_search_light = false\n"
      "        for _, technology in ipairs(btech.unit.technologies(unit)) do\n"
      "          if technology.code == btech.unit.technology.SEARCH_LIGHT "
      "then\n"
      "            assert(technology.name == \"SearchLight\"\n"
      "              and technology.group == \"primary\")\n"
      "            found_search_light = true\n"
      "          end\n"
      "        end\n"
      "        assert(found_search_light)\n"
      "        assert(select(\"#\", "
      "btech.unit.remove_technology(unit, btech.unit.technology.SEARCH_LIGHT)) "
      "== 0)\n"
      "        local secondary = btech.unit.technology.TARGETING_COMPUTER\n"
      "        assert(select(\"#\", btech.unit.add_technology(unit,\n"
      "          secondary)) == 0)\n"
      "        local returned_secondary\n"
      "        for _, technology in ipairs(btech.unit.technologies(unit)) do\n"
      "          if technology.code == secondary then\n"
      "            assert(technology.group == \"secondary\"\n"
      "              and technology.source == \"configured\")\n"
      "            returned_secondary = technology.code\n"
      "          end\n"
      "        end\n"
      "        assert(returned_secondary == secondary)\n"
      "        assert(select(\"#\", btech.unit.remove_technology(unit,\n"
      "          returned_secondary)) == 0)\n"
      "        for _, technology in ipairs(btech.unit.technologies(unit)) do\n"
      "          assert(technology.code ~= secondary)\n"
      "        end\n"
      "        assert(select(\"#\", btech.unit.add_technology(unit,\n"
      "          returned_secondary)) == 0)\n"
      "        local readded_secondary = false\n"
      "        for _, technology in ipairs(btech.unit.technologies(unit)) do\n"
      "          if technology.code == returned_secondary then\n"
      "            readded_secondary = technology.source == \"configured\"\n"
      "          end\n"
      "        end\n"
      "        assert(readded_secondary)\n"
      "        assert(select(\"#\", btech.unit.remove_technology(unit,\n"
      "          returned_secondary)) == 0)\n",
      file);
  fputs(
      "        assert(select(\"#\", "
      "btech.unit.clear_technologies(unit, "
      "btech.unit.technology_groups.INFANTRY)) == 0)\n"
      "        ok, err = mux.error.pcall(btech.unit.clear_technologies, unit, "
      "\"infantry\")\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2)\n"
      "        assert(select(\"#\", btech.unit.reset_critical_slots(unit)) "
      "== 0)\n"
      "        local laser = assert(btech.parts.search(\"*MediumLaser*\")[1])\n"
      "        local large_laser = assert(\n"
      "          btech.parts.search(\"*LargeLaser*\")[1])\n"
      "        local ac = assert(btech.parts.search(\"*AC/20*\")[1])\n"
      "        ok, err = mux.error.pcall(btech.unit.install_weapon, unit, {\n"
      "          part = laser, section = btech.unit.sections.LEFT_SIDE,\n"
      "          slots = { 1 }\n"
      "        })\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2 and #btech.unit.weapons(unit) == "
      "0)\n"
      "        ok, err = mux.error.pcall(btech.unit.install_weapon, unit, {\n"
      "          part = large_laser, section = left_arm, slots = { 1 }\n"
      "        })\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2 and #btech.unit.weapons(unit) == "
      "0)\n"
      "        ok, err = mux.error.pcall(btech.unit.install_weapon, unit, {\n"
      "          part = ac, section = left_arm, slots = {}\n"
      "        })\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2 and #btech.unit.weapons(unit) == "
      "0)\n"
      "        ok, err = mux.error.pcall(btech.unit.install_weapon, unit, {\n"
      "          part = ac, section = left_arm, slots = { 1, 1 }\n"
      "        })\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2 and #btech.unit.weapons(unit) == "
      "0)\n"
      "        ok, err = mux.error.pcall(btech.unit.install_weapon, unit, {\n"
      "          part = laser, section = left_arm, slots = { \"bad\" }\n"
      "        })\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2 and #btech.unit.weapons(unit) == "
      "0)\n"
      "        ok, err = mux.error.pcall(\n"
      "          btech.unit.add_technology, unit, \"SearchLight\")\n"
      "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
      "          and err.detail.argument == 2)\n"
      "        ok, err = mux.error.pcall(btech.unit.add_technology, unit,\n"
      "          btech.unit.technology.SWARM_ATTACK)\n"
      "        assert(not ok and err.code == \"mux.object.invalid\"\n"
      "          and err.detail.argument == 1)\n",
      file);
  fputs("        assert(select(\"#\", btech.unit.install_weapon(unit, {\n"
        "          part = laser, section = left_arm, slots = { 1 }\n"
        "        })) == 0)\n"
        "        assert(btech.unit.weapons(unit, left_arm)[1].section == "
        "left_arm)\n"
        "        assert(select(\"#\", btech.unit.set_weapon_modes(unit, 0, {\n"
        "          fire_modes = { btech.unit.fire_modes.REAR_MOUNT }\n"
        "        })) == 0)\n"
        "        local criticals = btech.unit.critical_slots(unit, left_arm)\n"
        "        assert(criticals[1].section == left_arm\n"
        "          and criticals[1].fire_modes[1]\n"
        "            == btech.unit.fire_modes.REAR_MOUNT)\n"
        "        ok, err = mux.error.pcall(btech.unit.set_weapon_modes, unit, "
        "0, {\n"
        "          fire_modes = { \"RearMount\" }\n"
        "        })\n"
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 3)\n"
        "        ok, err = mux.error.pcall(btech.unit.set_weapon_modes, unit, "
        "0, {\n"
        ...
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 3)\n"
        "        ok, err = mux.error.pcall(btech.unit.configure_ammunition,\n"
        "          unit, { weapon = ac, section = left_arm, slot = 2,\n"
        "            ammunition_modes = { \"Inferno\" } })\n"
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 2)\n"
        "        assert(select(\"#\", btech.unit.configure_ammunition(unit, {\n"
        "          weapon = ac, section = left_arm, slot = 2,\n"
        "          ammunition_modes = { btech.unit.ammunition_modes.INFERNO }\n"
        "        })) == 0)\n"
        "        criticals = btech.unit.critical_slots(unit, left_arm)\n"
        "        assert(criticals[2].ammunition_modes[1]\n"
        "          == btech.unit.ammunition_modes.INFERNO)\n"
        "        assert(select(\"#\", "
        "btech.unit.restock_ammunition(unit, left_arm, 2)) == 0)\n"
        "        assert(select(\"#\", btech.unit.install_special(unit, {\n"
        "          section = left_arm, slot = 3\n"
        "        })) == 0)\n"
        "        assert(select(\"#\", "
        "btech.unit.save_template(unit, \"Lua-Admin-Test\")) == 0)\n"
        "        assert(select(\"#\", btech.unit.restore(unit)) == 0)\n"
        "        assert(select(\"#\", btech.unit.set_unit_type(unit, "
        "btech.unit.types.VEHICLE))\n"
        "          == 0)\n"
        "        local vehicle_rear =\n"
        "          btech.unit.armor(unit, "
        "btech.unit.sections.FRONT_SIDE).rear_armor.current\n"
        "        ok, err = mux.error.pcall(btech.repair.apply, unit, {\n"
        "          operation = btech.repair.operations.REPAIR_REAR_ARMOR,\n"
        "          section = btech.unit.sections.FRONT_SIDE, value = "
        "7\n"
        "        })\n"
        "        assert(not ok and err.code == \"mux.arg.invalid\"\n"
        "          and err.detail.argument == 2\n"
        "          and btech.unit.armor(unit, "
        "btech.unit.sections.FRONT_SIDE).rear_armor.current\n"
        "            == vehicle_rear)\n"
  ...
  fputs("    {\n"
        ...
        "        assert(select(\"#\", "
        "btech.map.set_cargo_transfer_point(parent_dbref,\n"
        "          { x = 1, y = 2, reveal_hint = true })) == 0)\n"
        ...
        "          } }\n"
        "        assert(select(\"#\", btech.map.set_link(child_dbref, "
        "link_value)) == 0)\n"
        ...
        "        assert(select(\"#\", btech.map.set_link(child, nil)) == 0)\n"
        "        assert(btech.map.link(child) == nil)\n"
        "        assert(select(\"#\", "
        "btech.map.set_cargo_transfer_point(parent, nil)) == 0)\n"
        "        assert(btech.map.cargo_transfer_point(parent) == nil)\n"
...
static int create_styled_object(int socket_fd) {
  ...
  if (send_command(socket_fd, "GOD\r\n") < 0 ||
      ...
      send_command(socket_fd, "la LuaAliasWorks\r\n") < 0 ||
      expect_text(socket_fd, "LuaAliasWorks") < 0 ||
...
static int check_styled_object(const char *directory) {
  ...
  if (sqlite3_open_v2(database_path, &database, SQLITE_OPEN_READONLY,
      ...
      sqlite3_prepare_v2(
          ...
          ", (SELECT link FROM objects"
          "   WHERE name = 'Lua Persistently Homed Thing')"
          ", (SELECT dbref FROM objects"
          "   WHERE name = 'Lua Link Destination One')"
  ...
  if (!strcmp((const char *)sqlite3_column_text(statement, 0),
      ...
      !strcmp((const char *)sqlite3_column_text(statement, 18),
              "example.lua") &&
      !strcmp((const char *)sqlite3_column_text(statement, 19), ""))
    result = 0;
